...
5. **`ct2-runtime-doctor-workflow-probe`** (P1) — Extend `ct2-runtime-doctor` with a network-free workflow-class probe (Workflow tool / ToolSearch-deferred-tool presence). Capability-gated, not version-gated; absence ⇒ serial fallback, never error. Surface in `ct2-status --doctor`. [Resolution 2026-06-10: shipped as a version-derived hint (`authority: hint`) — no directly observable workflow surface exists for a network-free shell probe; capability gating proper resumes once one ships.]
