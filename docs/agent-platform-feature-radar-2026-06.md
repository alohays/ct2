...
| Capability | Current state | Maturity | CT2 opportunity | Priority |
|---|---|---|---|---|
| **Stable release line** | `0.130.0` → `0.137.0` (2026-06-04); `0.138.0` alpha-only. Seven stable minors in the window. | stable | Re-pin documented Codex minimum to `0.136.0` (README still says `0.128.0+` at `README.md:480`; `MIN_CODEX` in doctor is `0.130.0`). Version-gate everything below. *Decision 2026-06-10 — kept `0.130.0` per capability-not-version gating (`spec/runtime-capabilities.md`); `0.136.0` surfaces are tracked as capabilities, not a version gate.* | **P0 maintain** |
