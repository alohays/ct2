...
fatal: files that are unreadable, undecodable, unparseable, or not JSON
objects are skipped; records carrying neither `fanout_width` nor
`agent_count`, or carrying non-numeric or non-finite values (booleans, `NaN`,
and infinities included) in any of the numeric fields (`fanout_width`,
`agent_count`, `wall_ms`, `agent_ms_total`), are skipped; individually
missing optional fields are tolerated as absent. Both tools
